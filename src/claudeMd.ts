...
export const BROWSER_RULES = [
  ...
  "- **Se der \"Browser extension is not connected\":** você resolve, sem depender da Claude Maia e **sem abrir janela nova**. Chrome fechado (`pgrep -x \"Google Chrome\"` vazio): `open -a \"Google Chrome\"`, espere ~4s e repita a chamada. Chrome aberto: abra a URL de reconexão da extensão numa aba da janela que já existe (a extensão intercepta, reconecta e fecha a aba sozinha; não rouba o foco): `osascript -e 'tell application \"Google Chrome\"' -e 'set w to window 1' -e 'set a to active tab index of w' -e 'make new tab at end of tabs of w with properties {URL:\"https://clau.de/chrome/reconnect\"}' -e 'delay 2' -e 'set active tab index of w to a' -e 'end tell'`, ~4s e repita. **NUNCA feche/reinicie o Chrome** (`quit app`, `killall`, fechar janelas que não são suas) nem crie janela nova: eu posso estar usando; se nem assim conectar, me avise em uma linha e pare.",
