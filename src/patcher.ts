...
export const PATCHES: readonly Patch[] = [
  {
    id: "autoBrowser",
    title: "navegador (Claude in Chrome) liga sozinho na primeira mensagem (uma vez por chat)",
    file: "webview/index.js",
    find: "let z=Y?.expandMentions!==!1,q=await LT1($,J,G,",
    replace:
      'let z=Y?.expandMentions!==!1;if(z&&!this.__maiaChromeTried&&(this.config?.value?.browserIntegrationSupported??!1)&&this.chromeMcpState?.value?.status==="disconnected"){this.__maiaChromeTried=!0;try{await this.ensureChromeMcpEnabled()}catch(_){}}let q=await LT1($,J,G,',
    marker: "this.__maiaChromeTried=!0;try{await this.ensureChromeMcpEnabled()}"
  },
  {
    id: "contextInChat",
